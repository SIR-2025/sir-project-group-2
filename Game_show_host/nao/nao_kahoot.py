...
NAO_IP = "10.0.0.37"  # Your Nao's IP address
...
if not TEST_API_ONLY:
    ...
    from sic_framework.devices.common_naoqi.naoqi_motion import (
        ...
        NaoqiMoveToRequest,   # <--- added for pacing
    )
...
class NaoQuizMaster(SICApplication):
    ...
    def __init__(self, nao_ip: str, server_url: str, test_mode: bool = False):
        ...
        # low-level NAOqi proxies (if available)
    ...
    # ----------------------------------------------------------------------
    # BASIC SPEECH (ALWAYS WITH MIC POSE)
    # ----------------------------------------------------------------------
    def _say_raw(self, text: str):
        """
        Low-level speech: just TTS (no mic pose).
        """
    ...
    def say(self, text: str):
        """
        High-level speech: start mic pose, speak, then end mic pose.
        This is used EVERY time the robot speaks.
        """
        # Always show text in console
        print(f"[NAO SAYS] {text}")
        ...
        # If we don't have low-level motion, fall back to a simple gesture
        ...
        # Full NAOqi path: mic pose start -> speak -> mic pose end
        try:
            self.mic_pose_start()
            ...
            # Hold pose for approximate speech duration
    ...
    def mic_pose_start(self):
        ...
        try:
            ...
            # Disable left arm swinging during walk, keep right arm normal
            try:
                self.motion_proxy.setWalkArmsEnabled(False, True)
            except Exception as e:
                print(f"[NAO] WARNING: setWalkArmsEnabled failed: {e}")
    ...
    def mic_pose_end(self):
        ...
        try:
            ...
            # Re-enable both walk arms
            try:
                self.motion_proxy.setWalkArmsEnabled(True, True)
            except Exception as e:
                print(f"[NAO] WARNING: setWalkArmsEnabled restore failed: {e}")
    ...
    def show_quiz_generating_start(self):
        ...
        """
        self.say("I'm generating your quiz, please wait!")
    ...
    def show_quiz_generating_end(self, success: bool, topic: str):
        ...
        if success:
            ...
            self.say(f"We have a {topic} quiz, let's go!")
        else:
            self.set_led_color(255, 0, 0, 0.5)
            self.say("I couldn't generate your quiz properly. I'll use the default questions.")
    ...
    def turn_left_right(self, angle_degrees: float = 180.0):
        """
        Pacing-style turn using SIC's NaoqiMoveToRequest.
        Turn a bit left, then back right.
        """
        if self.test_mode or not self.nao:
            print("[NAO] TEST MODE: Simulating pacing turn")
            return
        ...
        try:
            ...
            # Turn left
            self.nao.motion.request(
                NaoqiMoveToRequest(x=0.0, y=0.0, theta=theta)
            )
            time.sleep(0.2)

            # Turn back right to original orientation
            self.nao.motion.request(
                NaoqiMoveToRequest(x=0.0, y=0.0, theta=-theta)
            )
            time.sleep(0.2)
    ...
    def volume_check(self, threshold=3500, listen_seconds=3):
        ...
        self.say("Are we all ready? Make some noise!")
        time.sleep(0.5)
    ...
    def announce_question(self, question_data: Dict):
        ...
        for idx, option in enumerate(options):
            # pacing turn while listing options
    ...
    def run_quiz(self):
        ...
        self.say("Before we start, what topic should the quiz be about?")
