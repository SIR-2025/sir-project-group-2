...
@app.route('/api/set_quiz', methods=['POST'])
def api_set_quiz():
    """
    API endpoint for Nao/LLM to set a new quiz dynamically.
    ...
    The internal QUESTIONS list is replaced with this new quiz, converted
    ...
    if not data or "questions" not in data:
        return jsonify({"success": False, "message": "Missing 'questions' field"}), 400

    questions_in = data.get("questions", [])
    if not isinstance(questions_in, list) or len(questions_in) == 0:
        return jsonify({"success": False, "message": "Questions must be a non-empty list"}), 400
    ...
    for i, q in enumerate(questions_in):
        ...
        if not isinstance(text, str) or not isinstance(options, list) or len(options) != 4:
            return jsonify({"success": False, "message": f"Invalid question format at index {i}"}), 400
        if not isinstance(correct_index, int) or not 0 <= correct_index < 4:
            return jsonify({"success": False, "message": f"Invalid correct_index at index {i}"}), 400
    ...
    # Replace the global QUESTIONS list with the new one
    QUESTIONS[:] = new_questions

    # Reset quiz state to start fresh with new questions
    reset_state()

    return jsonify({"success": True, "message": "Quiz set from NAO/LLM.", "total_questions": len(QUESTIONS)})
...
def api_reset():
    ...
    Does NOT restore original QUESTIONS; it keeps whatever quiz
    is currently loaded (either from quiz_data or from /api/set_quiz).
    """
    reset_state()
